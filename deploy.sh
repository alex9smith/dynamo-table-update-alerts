...
sam deploy --template-file infra/alerter.yaml --stack-name dynamo-alerts-alerter --capabilities CAPABILITY_IAM --parameter-overrides TableStreamArn=$TABLE_STREAM_ARN TopicArn=$TOPIC_ARN TableName=$TABLE_NAME